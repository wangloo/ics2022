...
csr_reg(word_t field)
{
  switch (field) {
  case 0x341: return &(cpu.csr.mepc);
  case 0x342: return &(cpu.csr.mcause);
  case 0x300: return &(cpu.csr.mstatus.val);
...
// NEMU检测到ECALL执行后，应当根据CPU现在的状态去判断如何设置 mcause
// 目前来说，只有yield()会执行ecall指令，而当时的状态就是11: M 模式环境调用
// 所以就简化了实现，只支持在M模式下调用ECALL，行为才正确。
// abstract-machine/am/src/riscv/nemu/cte.c: yield()
#define ECALL(dnpc)  {dnpc = isa_raise_intr(11, s->pc);} 

#define MRET(dnpc) {dnpc = cpu.csr.mepc; cpu.csr.mstatus.part.MIE = cpu.csr.mstatus.part.MPIE; cpu.csr.mstatus.part.MPIE = 1; }
...
#define immB() do { *imm = SEXT(BITS(i, 31, 31), 1) << 11 | ((SEXT(BITS(i, 7, 7), 1) << 63) >> 63) << 10 \
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, uint64_t a=src1, b=src2; R(rd) = BITS(a * b, 63, 32)); 
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, int32_t a=src1, b=src2; R(rd) = BITS((int64_t)a*(int64_t)b, 63, 32));;
...
// static char *
// print_inst_format(uint32_t inst)
// {
...
//   memset(str, '0', 36);
//   for (; idx >= 0;) {
//     if ((idx == 28) || (idx == 22) || (idx == 18) || (idx == 12)) 
//       str[idx--] = ' ';
//     str[idx--] = (inst % 2) + '0';
//     inst /= 2;
//   }
...
int isa_exec_once(Decode *s) {
  ...
  // Log("Try to decoding %s\n", print_inst_format(s->isa.inst.val));
  // Log("Try to decoding %08x\n", s->isa.inst.val);
